...
<body>
    ...
    <form id="moveForm">
        ...
        <label for="inputDate">입력날짜:</label>
        <input type="text" id="inputDate" readonly>
        
        <label for="inputTime">입력시간:</label>
        <input type="text" id="inputTime" readonly>
    ...
    <script>
        ...
        document.getElementById("moveForm").addEventListener("submit", async function (event) {
            ...
            const now = new Date();
            const inputDate = `${now.getFullYear()}년 ${now.getMonth() + 1}월 ${now.getDate()}일`;
            const inputTime = `${now.getHours()}시 ${now.getMinutes()}분`;
            ...
            // POST request를 보냄
            const response = await fetch('https://script.google.com/macros/s/AKfycbwdADDtCDRLv4STiNDe-1ElbYmhRnPFbfmvx-4SqB4xEWuVHS3ySqfx_1T927-mc0MaPA/exec', {
                method: 'POST',
